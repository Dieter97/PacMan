...
void Game::restart(Game *g) {
    ...
    g->ui->addTextView("score", g->factory->createTextView(0, g->mapHeigth, "Score: 0", 12));
    g->ui->addTextView("lives", g->factory->createTextView(g->mapWidth - 6, g->mapHeigth, "Lives:3", 12));
    g->start();
}
...
void Game::resetLevel(Game *g) {
    ...
    g->ui->addTextView("start", g->factory->createTextView(g->mapWidth / 2 - 6.5f, g->mapHeigth / 2 - 3.2f,
                                                           "Press space to start", 18));
    g->ui->addTextView("score", g->factory->createTextView(0, g->mapHeigth, "Score: " + std::to_string(g->points), 12));
    g->ui->addTextView("lives", g->factory->createTextView(g->mapWidth - 6, g->mapHeigth, "Lives: " + std::to_string(g->lives), 12));
    g->start();
}
