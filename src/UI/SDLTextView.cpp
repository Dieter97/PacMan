...
void SDLTextView::visualize() {
    ...
    Message_rect.x = (int) floor((float)(posX * context->getTilewidth() * (context->getSCALE_FACTOR()))) + context->getX_offset();
    Message_rect.y = (int) floor((float)(posY * context->getTileHeigth() * (context->getSCALE_FACTOR())))+context->getY_offset();
